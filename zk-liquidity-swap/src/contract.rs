...
pub struct ContractState {
    ...
    /// Whether the contract is operable or not. TokenBalances can still be withdraw when closed.
    pub is_closed: bool,
    /// User balances.
...
impl ContractState {
    ...
    fn get_pools(&self) -> &TokenBalance {
        self.balances.get_balance_for(&self.token_pool_address)
    }
...
pub fn initialize(
    ...
    let mut balances = TokenBalances::new(token_pool_address, token_a_address, token_b_address);
    balances.add_to_token_balance(token_pool_address, Token::A, 0);
...
pub fn provide_liquidity(
    ...
    let to_token = state.balances.deduce_tokens_in_out(token_address).token_in;

    state.balances.move_tokens(
        context.sender,
    ...
    if pool_balance.get_amount_of(Token::A) > 0 && pool_balance.get_amount_of(Token::B) > 0 {
        let swap_constant = pool_balance
            .get_amount_of(Token::A)
            .checked_mul(pool_balance.get_amount_of(Token::B));
...
pub fn deposit(
    ...
    let from_token = state.balances.deduce_tokens_in_out(token_address).token_in;
...
#[zk_on_secret_input(shortname = 0x13, secret_type = "SecretAmountAndDirection")]
pub fn swap(
    context: ContractContext,
    state: ContractState,
    zk_state: ZkState<SecretVarMetadata>,
    ...
    let balance = state.balances.get_balance_for(&context.sender);
    assert!(
...
pub fn swap_variable_inputted(
    ...
    let swap_sender = secret_var_info.owner;
...
fn calculate_token_to_amount(
    ...
    let pool_balance = state.get_pools();
    let from_pool_value = pool_balance.get_amount_of(token_from);
    let to_pool_value = pool_balance.get_amount_of(token_to);
...
pub fn withdraw(
    ...
    let token_from = state.balances.deduce_tokens_in_out(token_address).token_in;

    state
        .balances
        .deduct_from_token_balance(context.sender, token_from, amount);
...
pub fn close_pools(
    ...
    state.balances.move_tokens(
        ...
        Token::A,
        pool_balance.get_amount_of(Token::A),
    );
    state.balances.move_tokens(
        state.token_pool_address,
        state.contract_owner,
        Token::B,
        pool_balance.get_amount_of(Token::B),
    );
    ...
    let pool_balance = state.get_pools();
    assert_eq!(pool_balance.get_amount_of(Token::A), 0);
    assert_eq!(pool_balance.get_amount_of(Token::B), 0);
